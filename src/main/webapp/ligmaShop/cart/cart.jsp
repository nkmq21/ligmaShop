                ...
                <div class="col l-12 m-12 c-12">
                    ...
                    <c:if test="${empty cartItems}">
                        <div class="cart__empty">
                            <img src="${pageContext.request.contextPath}/resource/images/no-cart.jpg" alt="No items"
                                 class="cart__empty-img">
                            <!--                                    <p>Giỏ hàng của bạn đang trống!</p>-->
                            <a href="${pageContext.request.contextPath}/products" class="btn btn--primary">Quay lại mua
                                sắm</a>
                        </div>
                    </c:if>
                    ...
                    <c:if test="${not empty cartItems}">
                        <div class="cart__items">
                                    ...
                                    <tr>
                                            ...
                                            <div class="cart__item-info">
                                                <c:choose>
                                                    <c:when test="${empty item.productSizeColorID || empty item.productSizeColorID.productID || empty item.productSizeColorID.productID.productimagesCollection}">
                                                        <img src="${pageContext.request.contextPath}/resource/images/user.jpg"
                                                             class="cart__item-img">
                                                    </c:when>
                                                    <c:otherwise>
                                                        <c:forEach var="image"
                                                                   items="${item.productSizeColorID.productID.productimagesCollection}"
                                                                   varStatus="imgStatus">
                                                            <c:if test="${imgStatus.index == 0}">
                                                                <img src="${image.imageURL}"
                                                                     alt="${item.productSizeColorID.productID.productName}"
                                                                     class="cart__item-img">
                                                            </c:if>
                                                        </c:forEach>
                                                    </c:otherwise>
                                        ...
                                        <td>
                                            <form action="${pageContext.request.contextPath}/cart" method="post"
                                                ...
                                                <select name="productSizeColorID" class="cart__item-size">
                                                    <c:forEach var="sizeColor"
                                                               items="${item.productSizeColorID.productID.productsizecolorCollection}">
                                                        <option value="${sizeColor.productSizeColorID}"
                                                            ${sizeColor.productSizeColorID == item.productSizeColorID.productSizeColorID ? 'selected' : ''}>
                                                                ${sizeColor.sizeID.sizeName}
                                                        </option>
                                                    </c:forEach>
                                                </select>
                                                <input type="hidden" name="quantity" value="${item.quantity}">
                                                <button type="submit" class="update-size-btn">Cập nhật</button>
                                            </form>
                                        </td>
                                        <td>
                                            <fmt:formatNumber value="${adjustedPrice}" type="number"
                                                              groupingUsed="true"/> đ
                                        </td>
                                        <td>
                                            <form action="${pageContext.request.contextPath}/cart" method="post"
                                                ...
                                                <input type="number" name="quantity" min="1" value="${item.quantity}"
                                                       class="cart__item-quantity" required>
                                                <button type="submit" class="cart__update-btn btn">Cập nhật</button>
                                            </form>
                                        </td>
                                        <td>
                                            <fmt:formatNumber value="${adjustedPrice * item.quantity}" type="number"
                                                              groupingUsed="true"/> đ
                                        ...
                                        <td>
                                            <form action="${pageContext.request.contextPath}/cart" method="post"
                                                ...
                                                <button type="submit" class="cart__item-remove btn btn--danger">Xóa
                                                </button>
                                            </form>
                                        </td>
                                    </tr>
                            ...
                            <a href="${pageContext.request.contextPath}/ligmaShop/cart/checkout.jsp"
                               class="btn btn--primary cart__checkout-btn">Thanh Toán</a>
                        </div>
                    </c:if>
