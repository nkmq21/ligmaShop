...
//                  
    ...
    private void updateCartItem(HttpServletRequest request, Cart cart) {
        try {
            ...
            String productSizeColorIDParam = request.getParameter("productSizeColorID");
            Collection<Cartitems> cartItems = cart.getCartitemsCollection();
            if (cartItems == null) {
            ...
            for (Cartitems item : cartItems) {
                if (item.getCartItemID().equals(cartItemId)) {
                    if (newQuantity > 0) {
                        ...
                        if (productSizeColorIDParam != null && !productSizeColorIDParam.isEmpty()) {
                            int newProductSizeColorID = Integer.parseInt(productSizeColorIDParam);
                            EntityManager em = DBConnection.getEntityManager();
                            try {
                                Productsizecolor newProductSizeColor = em.find(Productsizecolor.class, newProductSizeColorID);
                                if (newProductSizeColor != null) {
                                    item.setProductSizeColorID(newProductSizeColor);
                                }
                            } finally {
                                em.close();
                            }
